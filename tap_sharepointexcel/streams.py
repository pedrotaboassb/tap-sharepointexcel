...
class DriveItemsStream(sharepointexcelStream):
    ...
    def get_child_context(self, record: dict, context: dict) -> dict:
        ...
        list_of_master_file_data = sorted(find_newest_file(record['value'], objs = []), key = lambda x: x['lastModifiedDateTime'])
...
class ExcelFile(sharepointexcelStream):
    ...
    def parse_response(self, response: requests.Response):
        ...
        clean_dict = pd.DataFrame.from_dict(delete_row(excel_data_dict, columns, row_index_list)).fillna(np.nan).replace(np.nan, None).to_dict('records') 
        
        json_final_data = json.dumps(clean_dict, default=serialize_datetime)
        json_final_data = json.loads(json_final_data)
